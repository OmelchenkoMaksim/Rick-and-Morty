...
class AlertDialogCharacterTypesFragment : DialogFragment() {

    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {

        val types: Array<String> = if (online) {
            FiltersOnline.charactersTypesOnline.toTypedArray()
        } else {
            DataBase.charactersTypesOffline.toTypedArray()
        }

        return activity?.let {
            val builder = AlertDialog.Builder(it)
            builder.setTitle(getString(R.string.types_characters_alert_title_fragment))
                .setSingleChoiceItems(
                    types, -1
                ) { _, item ->
                    DataBase.filterCharactersOptions[DataBase.CHARACTER_OPTION_TYPE] = types[item]
                }
                .setPositiveButton(
